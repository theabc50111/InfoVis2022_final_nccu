...
//d3.json("myColor.json").then(function(myColorData){
  //console.log(myColorData['rent'])

function spot_timeseries_click(e) {
  console.log(e);
  console.log(`${e.target.options.sno}, ${e.target.options.sna} has been click`);
  
  d3.json("./dataset/spot_timeseries.json")
    .then(function(data){
    ...
    chart = LineChart(data[`${e.target.options.sno}`], {
      ...
      y: d => d.count,
      z: d => d.division,
      yLabel: `Rent & Return of Ubike Station: ${e.target.options.sna}`,
      width: 500,
      height: 150,
      color: "steelblue"      
...
d3.json("./dataset/spot_timeseries.json")
    .then(function(data){
    chart = LineChart(data[141], {
      x: d => new Date(d.datetime),
      y: d => d.count,
      z: d => d.division,
      yLabel: "Rent & Return of Ubike Station: 文山行政中心",
      width: 500,
      height: 150,
      color: "steelblue"      
      //,  voronoi // if true, show Voronoi overlay      
